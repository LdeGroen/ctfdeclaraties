...
import { API_URL, verwerkInlogTerugkeer } from 'ctf-ui/api';
...
function App() {
    ...
    // De backend stuurt na het inloggen een eenmalige code terug in plaats van
    // het token zelf; die moet worden ingewisseld. Dat zit sinds ctf-ui v0.3.0
    // in het gedeelde pakket, juist omdat deze app er ooit op strandde: hij las
    // alleen ?token= en zette je daardoor terug op het inlogscherm.
    useEffect(() => {
        let afgebroken = false;
        (async () => {
            const uit = await verwerkInlogTerugkeer();
            if (afgebroken || uit.status === 'geen') return;
            if (uit.status === 'ok') {
                localStorage.setItem(TOKEN_KEY, uit.token);
                setToken(uit.token);
            } else {
                setAuthFout(uit.melding);
            }
        })();
